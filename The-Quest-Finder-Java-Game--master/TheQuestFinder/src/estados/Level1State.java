...
/**
 * Clase encargada de hacer la ejecución de todo lo que sucede en el nivel 1
...
public class Level1State extends State{
	...
	/**
	 * Constructor de Level1State que recibe el administrador de estados para poder 
	...
	public void init() {
		...
		handler.addObject(new Puerta(18 * 32, 13*32, ID.Coleccionable, handler, gsm));
		handler.addObject(new Coin(7*32, (10*32) + 16, ID.Coleccionable, handler));
		handler.addObject(new Key(18*32, (5*32) + 16, ID.Coleccionable, handler));
		handler.addObject(new Chocolat(11*32, (2*32) + 16, ID.Coleccionable, handler));
		...
		handler.addObject(new Spider(15 * 32, 14 *32 , ID.Enemigo, handler));
		handler.addObject(new Slime(7 * 32, 10 *32 , ID.Enemigo, handler));
	}
